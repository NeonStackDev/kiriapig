...
<script>
	...
	$(document).ready(function () {
		function toggleFollowingFields() {
			const refValue = $('#ref_no').val().trim();
			const $formInputs = $('#ref_no').closest('form').find(':input');

			let passedRef = false;
			const afterRef = $formInputs.filter(function () {
				if (this.id === 'ref_no') {
					passedRef = true;
					return false;
				}
			...
			afterRef.prop('disabled', refValue === '');
		}

		// Initial toggle
		toggleFollowingFields();
    ...
    $('#supplier_id').on('change', function() {
      ...
      if (supplierId !== '') {
        ...
        var getProduct = $.getJSON(
            '/purchases/get_products',
            { location_id: $('#location_id').val(), term: null ,supplier_id: $('#supplier_id').val(), module: 'purchases_addpurchases'},
				function(res){
					const data = res.map(p => ({
						label: p.text,           // what appears in the dropdown
						value: p.text,           // what gets typed into the box
					...
					// $( "#search_products" ).autocomplete( "option", "source", data );
				}
        );

	  $("#search_products").off('focus').on('focus', function () {
