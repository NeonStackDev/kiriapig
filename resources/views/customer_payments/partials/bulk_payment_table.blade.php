...
@if (!empty($sells) && $sells->count() > 0)
    ...
    @if($sell->type == 'opening_balance')
        ...
        <!-- Show the row only if there is outstanding amount is greater than 0-->
            <tr>
                ...
                <td>{{ @num_format(abs($outstandingRow)) }}
                    {!! Form::hidden('outstanding['.$sell->transaction_id.']', $outstandingRow, ['id' => 'outstanding_amount_'.$sell->transaction_id, 'class' => 'form-control outstanding_'.$sell->transaction_id , 'data-id' => $sell->transaction_id ]) !!}
                </td>
    ...
    @elseif ($outstandingRow >= 1)
         <!-- Show the row only if there is outstanding amount is greater than 0-->
            <tr>
                @if($sell->type == "cheque_return")
                    @if(!empty($sell->cheque_date))
                        <td>{{ @format_date($sell->cheque_date)}}</td>
                    @else
                        <td>{{ @format_date($sell->transaction_date)}}</td>
                    @endif
            ...
            <td>
                {{ @num_format(abs($outstandingRow)) }}
                {!! Form::hidden('outstanding['.$sell->transaction_id.']',$sell->cheque_number != null ? abs($outstandingRow) : abs($outstandingRow), ['id' => 'outstanding_amount_'.$sell->transaction_id, 'class' => 'form-control outstanding_'.$sell->transaction_id , 'data-id' => $sell->transaction_id ]) !!}
            </td>
            <td class="interest_selection_checkbox hide">
                <div class="row">
                    <div class="col-md-6 ">
