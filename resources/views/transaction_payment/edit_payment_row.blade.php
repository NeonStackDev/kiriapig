...
@endphp
    ...
    @php
        $controller_action = $is_transaction
            ? action('TransactionPaymentController@updateChequeTransaction', [$payment_line->id])
            : action('TransactionPaymentController@update', [$payment_line->id]);
    @endphp

    {!! Form::open(['url' => $controller_action, 'method' => 'put', 'id' => 'transaction_payment_add_form', 'files' => true]) !!}
            ...
            <div class="col-md-4">
              <div class="form-group">
                ...
                <div class="input-group">
                  ...
                  {!! Form::text('paid_on', @format_date($payment_line->paid_on), ['class' => 'form-control','id' => 'paid_on_date', 'readonly', 'required']); !!}
                </div>
              </div>
            </div>
            @php
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <div class="input-group">
                        ...
                        {!! Form::select("account_id", $accounts, !empty($payment_line->account_id) ? $payment_line->account_id : '' , ['class' => 'form-control select2 account_id', 'id' => "account_id", 'selected' => "!empty($selectedAccount) && $payment_line->account_id == $selectedAccount) && ->id ? 'selected' : '' ", 'style' => 'width:100%;']); !!}
                      </div>
                    </div>
                  </div>
                  @endif
...
<script>
  @if(!empty($payment_line->cheque_date) && !empty($payment_line->method) && (strtolower($payment_line->method) == 'cheque' || strtolower($payment_line->method) == 'bank_transfer' || strtolower($payment_line->method) == 'bank'  || strtolower($payment_line->method) == 'direct_bank_deposit'))
  ...
  $('#paid_on_date').datepicker('setDate', '{{@format_date($payment_line->paid_on)}}');
