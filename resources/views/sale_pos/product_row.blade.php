...
<tr class="product_row" data-row_index="{{$row_count}}">
    ...
    @if($product->enable_stock && empty($pos_settings['allow_overselling']) && empty($is_sales_order))
        ...
        data-qty_available="{{$product->current_stock}}"
...
>
	...
	<td class="{{$hide_tax}}">
        <input type="hidden" value="{{@number_format($unit_price_inc_tax)}}" class="pos_unit_price_inc_tax" />
	...
	<td>
		...
		__('lang_v1.percentage')], 'fixed', ['class' => 'form-control  row_discount_type_table','onchange'=>'updateDiscount(this.name,this.value)', 'disabled' => isset($price_later) && $price_later ==
	...
	<td>
		...
		row_discount_amount_table','oninput'=>'updateDiscount(this.name,this.value)', 'disabled' => isset($price_later) && $price_later == 1 ? true : false]); !!}

	</td>

	@if (isset($pos_settings['show_purchase_price']) && ($pos_settings['show_purchase_price'] =='1'))
	<td class="price_later_td  @if(isset($price_later) && $price_later != 1) hide @endif">
		...
		<input type="text" name="products[{{$row_count}}][last_purchased_price]"
			...
			@if(empty($pos_settings['is_pos_subtotal_editable'])) readonly @endif
			value="{{@num_format(!empty($product->last_purchased_price) ? $product->last_purchased_price : $product->dpp_inc_tax)}}">
	...
	@if(isset($is_sales_page) && $is_sales_page == '1' && !request()->session()->get('is_sales_pos', false))

	<td>

		<input type="text" name="products[{{$row_count}}][tax]" @if(isset($price_later) && $price_later=='1' ) disabled @endif
	...
	<td>
		...
		<input type="text" name="products[{{$row_count}}][sell_price_inc_tax]" @if(isset($price_later) && $price_later=='1' ) disabled @endif
...
$(document).ready(function() {
    $(document).on('input', '.row_discount_type_table, .row_discount_amount_table, .pos_quantity, .input_number[name*="[unit_price_inc_tax]"], .tax', function() {
        var name = $(this).attr('name');
        var row = name.match(/\d+/)[0];
        var discountType = $("select[name='products[" + row + "][line_discount_type]'], select.row_discount_type_table").val();
        var discountValue = parseFloat($("input[name='products[" + row + "][line_discount_amount]'], input.row_discount_amount_table").val().replace(/[^0-9.-]+/g, "")) || 0;
        var priceIncTax = parseFloat($("input[name='products[" + row + "][unit_price_inc_tax]']").val().replace(/,/g, '').replace(/[^0-9.-]+/g, ""));
        var quantity = parseFloat($("input[name='products[" + row + "][quantity]']").val().replace(/[^0-9.-]+/g, "")) || 1;
        ...
        if (isNaN(priceIncTax) || isNaN(quantity)) {
            console.log('Invalid number detected:', {priceIncTax, quantity});
            return;
        }

        // Apply discount to price including tax
        var subtotal = priceIncTax * quantity;
        ...
        $("#subtotal_text_" + row).text(subtotal.toFixed(2));
        $("#subtotal_text_" + row).attr('title', `priceIncTax=${priceIncTax}, qty=${quantity}, discountType=${discountType}, discountValue=${discountValue}, subtotal=${subtotal}`);
    });

    window.updateDiscount = function(elmName, value) {
        var $elm = $(".modal-body [name='" + elmName + "']");
        $elm.val(value);
        var row = elmName.match(/\d+/)[0];
        $elm.trigger('change');
    };
});
</script>
