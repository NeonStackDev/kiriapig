	...
	<div class="box box-solid">
		<div class="box-body">
			<div class="row">
				...
				<div class="col-sm-4">
					<div class="form-group">
						{!! Form::label('stock_adjustment_type', __('stock_adjustment.stock_adjustment_type') . ':*') !!} 
						{!! Form::select('stock_adjustment_type', [ 'increase' =>  __('stock_adjustment.increase'), 'decrease' =>  __('stock_adjustment.decrease')],  null, ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'required']); !!}
					</div>
				</div>
			</div>
		</div>
				...
				<div class="col-sm-8 col-sm-offset-2">
					<div class="form-group">
						<div class="input-group">
							...
							<input type="hidden" id="module" value="stockadjustment_add">
...
@section('javascript')
    ...
    function updateStoreOptions(locationId) {
    ...
    // Event handler for location_id change
    $('#location_id').change(function () {
        updateStoreOptions($(this).val());
    });

    // Initial call on page load
    $(document).ready(function () {
        $('#location_id').trigger('change'); 
        updateStoreOptions('{{ array_keys($business_locations->toArray())[0] }}');
    });
	...
	$(document).on('input', '.product_unit_price', function() {
		let value = parseFloat($(this).val());
		if (value < 0) {
			$(this).val(0); // Reset to 0 if negative
		}
	});
	$(document).on('change', '#stock_adjustment_type', function() {
		update_table_total();
	});
</script>
