    			     ...
    			     <tr>
    			        ...
    			        <td>
    			            ...
    			            @if(!empty($one->invoice_no))
								<b>@lang('lang_v1.invoice_no')</b> {{$one->invoice_no}}
								@if($one->is_settlement_customer_payment)
									<br>
									Settlement No <b>{{$one->bill_no}}</b>
									<br>
    			...
    			@else
    			    <tr @if(!empty($one->deleted_at)) class="deleted-row" @endif>
    			        ...
    			        <td>
    			            ...
    			            @if(!empty($transaction) && !empty($transaction->ref_no) && ($one->type != "payment" || $one->transaction_type == "hms_booking") )
								@if($one->type == 'cheque_return')
									<b>Cheque Return Ref. No. {{ $transaction->ref_no }}</b>
								@elseif($one->transaction_type == "hms_booking")
