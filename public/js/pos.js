$(document).ready(function() {
        ...
        $('#search_product')
            .autocomplete({
                ...
                response: function(event, ui) {
                    if (ui.content.length == 1) {
                        ...
                        if ((ui.item.enable_stock == 1 && ui.item.qty_available > 0) || 
                                (ui.item.enable_stock == 0) || is_overselling_allowed || for_so || is_quotation) {
                            $(this)
                ...
                select: function(event, ui) {
                    ...
                    if (ui.item.enable_stock != 1 || ui.item.qty_available > 0 || is_overselling_allowed || for_so || is_quotation) {
                        $(this).val(null);
            ...
            if (item.enable_stock == 1 && item.qty_available <= 0 && !is_overselling_allowed && !for_so && !is_quotation) {
    ...
    $('table#pos_table tbody').on(
        'change',
        'select.row_discount_type, input.row_discount_amount, input.pos_quantity',
        function() {
            var tr = $(this).parents('tr');
            ...
            // here
            var original_unit_price_inc_tax = __read_number(tr.find('input.original_pos_unit_price_inc_tax'));
            var discount_value = original_unit_price_inc_tax - discounted_unit_price;
            
            __write_number(tr.find('input.pos_discount_value'), discount_value);
    ...
    if (add_via_ajax) {
        ...
        $.ajax({
            ...
            data: {
                ...
                is_quotation: (is_quotation ? 'yes' : 'no')
            },
            dataType: 'json',
            success: function(result) {
                if (result.success) {
...
function pos_each_row(row_obj) {
    ...
    __write_number(row_obj.find('input.pos_line_total'), line_total.toFixed(2));
    const currencyText = row_obj.find('.pos_line_total_text').text().replace(/[0-9.,\s]/g, '').trim();
    row_obj.find('.pos_line_total_text').text(`${currencyText} ${__currency_trans_from_en(line_total, false)}`)
    console.log({
        unit_price,
        discounted_unit_price,
        tax_rate,
        tax_value: __calculate_amount('percentage', tax_rate, discounted_unit_price),
        unit_price_inc_tax,
        discount,
        discount_type,
        line_total,
    ...
    row_obj.find('input.row_discount_amount_table').val(discount);
    row_obj.find('select.row_discount_type_table').val(discount_type);
}
...
function pos_discount(total_amount) {
    ...
    $('table#pos_table tbody tr').each(function() {
        var discounted_unit_price = calculate_discounted_unit_price($(this));
        var unit_price = __read_number($(this).find('input.pos_unit_price'));
        var qty = __read_number($(this).find('input.pos_quantity'));

        console.log(discounted_unit_price, unit_price, unit_price - discounted_unit_price, 'disc');
        row_discount += qty * (unit_price - discounted_unit_price);
    });

    $('span#total_discount').text(__currency_trans_from_en((discount+row_discount), false));
    return discount;
}
...
function pos_order_tax(price_total, discount) {
    ...
    $('table#pos_table tbody tr').each(function() {
        ...
        var discounted_unit_price = calculate_discounted_unit_price($(this));
        row_tax += ((tax_rate/100) * discounted_unit_price) * __read_number($(this).find('input.pos_quantity'));
    });

    $('span#order_tax').text(__currency_trans_from_en((order_tax+row_tax), false));
...
function calculate_discounted_unit_price(row) {
    ...
    if (row_discount_amount) {
        if (row_discount_type == 'fixed') {
            ...
            row_discounted_unit_price = __subtract_percent(this_unit_price, row_discount_amount);
        }
    }
