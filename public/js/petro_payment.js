...
$(document).on("click", ".delete_cash_payment", function () {
    ...
    let classes = tr.attr('class');
    let cust_pay_id = null
    if (typeof classes === 'string') {
        let match = classes.match(/paymt-(\d+)/);
        cust_pay_id = match ? match[1] : null;
    }

    console.log('iddd: ', cust_pay_id)
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success) {
                ...
                calculateTotal("#cash_table", ".cash_amount", ".cash_total");
                handlePaymentSuccess();
                cust_pay_id ? handlePaymentSuccessDelete(cust_pay_id) : ''
            } else {
                toastr.error(result.msg);
            }
        },
