...
class EssentialsEmployeesController extends Controller
{
    ...
    public function fetchLedgerSummarised(Request $request){
        ...
        $balance = $this->fetchBF($contact_id,$start_date,$end_date);
        $summary = $this->fetchSummary($contact_id,$start_date,$end_date);
        $summary['balance'] = number_format($balance, $businessCurrencyPrecise, '.', ',');
        $summary['balance_due'] = number_format($summary['income'] - $summary['paid'], $businessCurrencyPrecise, '.', ',');
   ...
   public function fetchEmployeeLedger(Request $request){
        ...
        $ob = EssentialsEmployeeAdvance::join('essentials_employees', 'essentials_employees.id', '=', 'essentials_employee_advances.employee_id' )
            ...
            ->orderBy('essentials_employee_advances.id','DESC')
            ->get();

        $balance = $this->fetchBF($emp_id,$start_date,$end_date);
        $previousBalance = $this->fetchBF($emp_id,$start_date,$end_date);
        ...
        return DataTables::of($ob)
            ...
            ->addColumn('description', function ($row) {
                ...
                $html .= '<span> <b>Liability or Expense Account :</b> '.$row->payment_method_name.'</span>'; // ✅ FIXED: dynamic account name
                return $html;
            })
            ...
            ->addColumn('balance', function ($row) use ($previousBalance, $businessCurrencyPrecise) {
                return number_format(abs($previousBalance+$row->amount - $row->amount_paid), $businessCurrencyPrecise, '.', ',');
            })
    ...
    public function fetchBF($contact_id,$start_date,$end_date){
