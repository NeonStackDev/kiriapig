								...
								<div class="table-responsive">
									...
									<table class="table table-bordered table-striped w-100" id="employees_table">
										...
										<tr>
											...
											<th width="20%">@lang( 'essentials::lang.e_payment_no' )</th>
										...
										<tr v-if="employees.length > 0" v-for="(emp,ind) in employees">
											...
											<td class="text-danger">
												<!--<input type="text" value="@{{ emp.e_payment_no }}" v-model="emp.e_payment_no" class="form-control" />-->
												<!--<input type="text" v-model="emp.e_payment_no" value="@{{ emp.e_payment_no }}" class="form-control" />-->
												<input type="text" value="@{{ emp.e_payment_no }}" v-model="emp.e_payment_no" class="form-control" />
											</td>
											<td>@{{ emp.name }}</td>
											<td class="text-danger">
											...
											<td class="text-danger">
												<input :readonly="payment_system.find(item => item.id == payment.payment_type_id)?.need_amount == 0" v-model="emp.amount"  class="form-control" @input="syncAmountPaid(emp)"/>
										...
										<tfoot>
										<tr>
											<th class="text-danger" colspan="4" align="right">Total</th>
								...
								<div class="table-responsive">
									...
									<table class="table table-bordered table-striped w-100" id="advances_table">
										...
										<tr>
											...
											<th>@lang( 'essentials::lang.e_payment_no' )</th>
							...
							@component('components.widget', ['class' => 'box-solid'])
								...
								<div class="table-responsive">
									...
									<table class="table table-bordered table-striped w-100" id="settings_table">
										...
										<tr>
											<th>@lang( 'messages.action' )</th>
											<th width="20%">@lang( 'essentials::lang.hrm_payment_settings_title_date' )</th>
											<th width="20%" >@lang( 'essentials::lang.hrm_payment_settings_title_type' )</th>
											<th width="20%" >@lang( 'essentials::lang.liable_expense_account' )</th>
											<th width="5%" >Enter value in the amount column</th>
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="form-group">
							...
							<input readonly="readonly" value="{{ now()->format('Y-m-d H:i') }}" class="form-control" name="datetime" type="text">
						</div>
						<div class="form-group">
		...
		new Vue({
			...
			data() {
				return {
					...
					advance: {},
					setting: { date: '', payment_type: '', liability_account_id: '', remarks: '', employee_ledger: '', need_amount: '' },
					advances: {!! json_encode($advances) !!},
					employees: {!! json_encode($employees) !!},
					business: {!! json_encode($business) !!},
					payment: { period: '{{ $startdate }} to {{ $enddate }}', date: '{{ $today }}', paid: "yes", check: '', account: '', payment_type_id: "", payment_method_id: "46" },
				...
				loadAllPaymentMethods() {
					...
					Promise.all(promises).then((results) => {
						...
						$('#payment_method_id')
								...
								.attr('required', true)
								.val($('#payment_method_id option:first').val()) // 👈 Select first option
								.trigger('change');
