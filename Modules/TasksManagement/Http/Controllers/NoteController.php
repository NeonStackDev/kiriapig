...
class NoteController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            $notes = Note::leftjoin('note_groups', 'notes.group_id', 'note_groups.id')
                ->leftjoin('task_management_statuses', 'notes.status_id', 'task_management_statuses.id')
                ->where(function ($q) use ($user_id) {
                    $q->where('created_by', $user_id)->orWhereJsonContains('shared_with_users', strval($user_id));
                })
                ->select([
                    'notes.*', 'note_groups.name as note_group',
                    'task_management_statuses.name as status_name',
                    'task_management_statuses.color as status_color'
                ]);
            ...
            return DataTables::of($notes)
                ...
                ->editColumn('status_name', function ($row) {
                    return $row->status_name ?? 'No Status';
                })
    ...
    public function create()
    {
        ...
        $count++;
        $statuses = TaskManagementStatus::where('business_id', $business_id)->pluck('name', 'id')->toArray();;
        $statuses_default = TaskManagementStatus::where('business_id', $business_id)->where('default', 1)->first();
        $statuses_default_id = $statuses_default?->id;
        $note_id = $count;

        return view('tasksmanagement::notes.create')->with(compact('note_groups', 'users', 'note_id', 'statuses', 'statuses_default_id'));
    }
    ...
    public function show($id)
    {
        $note = Note::leftjoin('note_groups', 'notes.group_id', 'note_groups.id')
            ...
            ->select([
                ...
                'task_management_statuses.name as status_name', 'task_management_statuses.color as status_color'
            ])->first();
