    ...
    public function getF22Form(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($purchases)
                ...
                ->addColumn('stock_count', function ($row) {
                    return '<input class="form-control stock_count"  name="f22[' . $row->product_id . '][stock_count]" id="f22[' . $row->product_id . '][stock_count]"  style="width: 80px;" name="stock_count" value="'.$row->alert_quantity.'" >';
                })
                ->addColumn('qty_difference', function ($row) {
