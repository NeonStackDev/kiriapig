          ...
          <div class="card-body">
            <form id="filters-form" class="form-row-custom">
              ...
              <div class="form-group-custom">
                  ...
                  {!! Form::date('transaction_date', $active_settlement ? date('Y-m-d', strtotime($active_settlement->transaction_date)): null, ['class' =>
            ...
            <div class="card-custom">
              ...
              <div class="card-body">
                <form id="cust-payments-form" class="form-row-custom align-items-end">
                  <div class="d-flex" style="width: 100%;">
                    ...
                    <div class="form-group-custom " style="flex: 0 0 17%; margin: 5px;">
                      ...
                      {!! Form::select('customer_payment_payment_method', ['cash' => 'Cash', 'card' => 'Card', 'cheque' => 'Cheque', 'bank' => 'Bank'],
                    ...
                    <div class="form-group-custom hide bank_div" style="flex: 0 0 12.5%; margin: 5px;">
                      {!! Form::label('customer_payment_account_module', __('lang_v1.payment_account').':') !!}
                      {!! Form::select('customer_payment_account_module', $account_modules_bank,
                      !empty($temp_data->customer_payment_payment_method) ? $temp_data->customer_payment_payment_method : null,
                      [
                      'class' => 'form-control select2',
                      'style' => 'width: 100%;',
                ...
                <ul class="list-unstyled-custom d-none" id="cust-payments-list">
                  <li>Customer… <span>{{  __('SettlementSW::lang.payment_method') }}</span> <span>Amount</span> 
  ...
  $('#settlement_print').css('visibility', 'hidden');
      ...
      @endif
        ...
        function calculateTotal_interne(table_name, class_name_td, output_element) {
            ...
            $(output_element).text(__number_f(total, false, false, currencyPrecision));
        }
                  ...
                  if (!result.success) {
                      ...
                      $("#expense-tbody").prepend(
                        ...
                        <tr> 
                            ...
                            <td class="sw_expense_amount">${formatWithCommasFixed(expense_amount, currencyPrecision)}</td>
                            <td>
      ...
      saveTempData();
        ...
        $(document).on('change', '#pump_closing_meter', function () {
          ...
          if (pump_closing_meter < pump_starting_meter) {
              ...
              sold_qty = (pump_closing_meter - pump_starting_meter); // changed from 6 to 3
    ...
    $(document).off('click', '.btn-modal').on('click', '.btn-modal', function(e) {
      ...
      sold_qty = sold_qty - testing_qty??0;
      ...
      let currencyPrecision2 = {{ $currency_precision }};
                            ...
                            // Append row to table (example)
                                $('#meter-sales-tbody').append(`
                                  <tr data-pump-id="${response?.data?.pump_id}">
                                      ...
                                      <td>${formatWithCommas(parseFloat(response?.data?.pump_start))}</td>
                                      <td>${formatWithCommas(parseFloat(response?.data?.pump_close))}</td>
                                      <td>${formatWithCommas(response?.data?.unit_price)}</td>
                                      <td><span class="sold_qty">${formatWithCommas(parseFloat(response?.data?.sold_qty, currencyPrecision))}</span></td>
                                      <td>${response?.data?.discount_type}</td>
                                      <td>${formatWithCommas(response?.data?.discount_val, currencyPrecision)}</td>
                                      <td>${formatWithCommas(response?.data?.testing_qty, currencyPrecision)}</td>
                                      <td>${formatWithCommas(response?.data?.total_qty, currencyPrecision)}</td>
                                      <td>${formatWithCommas(response?.data?.unit_price * response?.data?.sold_qty, currencyPrecision)}</td>
                                      <td>${formatWithCommas(response?.data?.after_discount, currencyPrecision)}</td>
                                      <td>
          ...
          if(item_id){
            $.ajax({
              ...
              success: function (result) {
                ...
                $('#other_sale_price').val(formatWithCommas(result.price, currencyPrecision));
              ...
              $('#other-sales-tbody').append(
                  `
                    <tr>
                        ...
                        <td>${formatWithCommasFixed(other_sale_price, currencyPrecision)}</td>
                        <td>${formatWithCommasFixed(other_sale_qty, currencyPrecision2)}</td>
                        <td>${capitalizeFirstLetter(other_sale_discount_type)}</td>
                        <td>${formatWithCommasFixed(other_sale_discount, currencyPrecision)}</td>
                        <td>${formatWithCommasFixed(sub_total, currencyPrecision)}</td>
                        <td>${formatWithCommasFixed(with_discount, currencyPrecision)}</td>
                        <td>
        ...
        function calculate_payment_tab_total(comingAmount = 0) {
          ...
          $('.payment_meter_sale_total').text(
                  __number_f(meter_sale_totals, false, false, currencyPrecision)
          );
          $('.payment_other_sale_total').text(
                  __number_f(other_sale_totals, false, false, currencyPrecision)
          );
          $('.payment_other_income_total').text(
                  __number_f(other_income_totals, false, false, currencyPrecision)
          );
          $('.payment_customer_payment_total').text(
                  __number_f(customer_payment_totals, false, false, currencyPrecision)
          );
          $('.meter_sale_total').text(__number_f(meter_sale_totals, false, false, currencyPrecision));
          $('.other_sale_total').text(__number_f(other_sale_totals, false, false, currencyPrecision));
          ...
          $('#amount_total').val(__number_f(all_totals, false, false, currencyPrecision));
          $('.total_amount').text(__number_f(all_totals, false, false, currencyPrecision));

          if(comingAmount > 0){
            add_payment(comingAmount)
          }
          ...
          $.ajax({
            ...
            success: function (result) {
              ...
              $('#cust-payments-list').append(`
                <li class="custum-name" data-id="${result.customer_payment_id}">
                ${result.customer_name} 
                <span>${result.payment_method}</span> 
                <span class="custum-val">${formatWithCommasFixed(result.amount, currencyPrecision)}</span>
                   
                <button type="button" class="btn btn-xs btn-danger sw_delete_cust_payment"
                            data-href="/petro/settlement/delete-customer-payment/${result.customer_payment_id}">
                          <i class="fa fa-times"></i>
                        </button>
                </li>
              `);
              ...
              $('#cust-payments-list .custum-name').each(function () {
                ...
                data.customer_payment_id = result.customer_payment_id
                data.account_type_payment = result.account_type_payment
                data.payment_method = result.payment_method

                if(payment_method == 'card' || payment_method == 'cash'){
                  updateCustomPaymentInAddPaymentForm(payment_method, result)
                }else{
                  updateCustomPaymentInAddPaymentForm(payment_method, data)
                }
              ...
              calculate_payment_tab_total(customer_payment_amount);
              updateBalance()
              saveTempData();
            },
          });
        ...
        function updateCustomPaymentInAddPaymentForm(paymentMethod, row){
          if(paymentMethod == 'cash'){
            $("#cash_table tbody").prepend(
                    `
                    <tr class="paymt-${row.customer_payment_id}"> 
                        <td>` +
                        row.customer_name +
                        `</td>
                        <td class="cash_amount">` +
                        __number_f(row.amount, false, false, currencyPrecision) +
                        `</td>
                        <td>
                          </td>
          ...
          }else if(paymentMethod == 'cheque' || paymentMethod == 'bank'){
        ...
        function deleteCustumePaymentDataId(id){
          console.log('LI: ', id)
          let li = $('#cust-payments-list').find('li[data-id="' + id + '"]');
          let url = li.find('button.sw_delete_cust_payment').data('href');
          var is_edit = $("#is_edit").val() ?? 0;

          let amountText = li.find('.custum-val').text();          
          let amount = parseFloat(amountText.replace(/,/g, ''));
          let cust_pay_id = li.data('id');
          
          deleteCustomerPaymer(url, is_edit, amount, cust_pay_id, li)
        }
            ...
            success: function (result) {
              ...
              if (!result.success) {
                ...
                if(result.total_credit_sales !== undefined){
                  $("#credit-sales-total").text(formatWithCommasFixed(result.total_credit_sales, currencyPrecision));
        ...
        function deleteCustomerPaymer(url, is_edit, amount, cust_pay_id, li, deleteReverse = false){
          $.ajax({
            method: 'delete',
            url: url,
            data: {is_edit},
        ...
        $(document).on('click', '.sw_delete_cust_payment', function () {
          ...
          let amountText = li.find('.custum-val').text();              // ex: "3,500.00"
          let amount = parseFloat(amountText.replace(/,/g, ''));
          let cust_pay_id = li.data('id');
          
          deleteCustomerPaymer(url, is_edit, amount, cust_pay_id, li, true)
        });
      ...
      $(document).ready(function () {
          ...
          meterSalesRows.forEach(row => {
            $('#meter-sales-tbody').append(`
              <tr data-pump-id="${row.pump_id}">
                ...
                <td>
                  <button class="btn btn-xs btn-danger delete_meter_sale" data-href="/settlement-sw/delete-meter-sale/${row.id}">
                    <i class="fa fa-times"></i>
                  </button>
                </td>
              </tr>
        ...
        @if(!empty($temp_data->other_income))
          ...
          other_income_list.forEach(row =>{
            ...
            <tr>
                      ...
                      <td>
                        <button type="button" class="btn btn-xs btn-danger sw_delete_other_income"
                            data-href="/petro/settlement/delete-other-income/${row.id}">
                          <i class="fa fa-times"></i>
                        </button>
                      </td>
                    </tr>
        ...
        @if(!empty($temp_data->cust_payments_list))
          ...
          cust_payments_list.forEach(row =>{
              ...
              // Append customer payment line
              $('#cust-payments-list').append(`
                <li class="custum-name" data-id="${row.id}">${row.customer_name} 
                <span>${row.payment_method}</span> 
                <span class="custum-val">${formatWithCommasFixed(row.amount, currencyPrecision)}</span>
                        <button type="button" class="btn btn-xs btn-danger sw_delete_cust_payment"
                            data-href="/petro/settlement/delete-customer-payment/${row.id}">
                          <i class="fa fa-times"></i>
                        </button>
                        </li>
              `);
...
<!-- script for new css style related to the doc 7425 -->
  ...
  <script>
    async function updateBalance(){
      ...
      $('#total_balance').val(__number_f(bal, false, false, currencyPrecision));
      $('.total_balance').text(__number_f(bal, false, false, currencyPrecision));
    ...
    $(document).on("custom:payment_deleted", function (e, row_id) {
        deleteCustumePaymentDataId(row_id)
    });
               ...
               add_payment(row.card_amount);
                $("#card_table_body").append(`
                    <tr>
                        ...
                        <td class="card_amount">${__number_f(row.card_amount, false, false, currencyPrecision)}</td>
                        <td>${row.slip_no}</td>
                        <td>${row.card_note}</td>
                        <td>
        ...
        @if(!empty($temp_data->cash_rows))
            ...
            cash_rows.forEach(row => {
                ...
                $("#cash_table_body").append(`
                    <tr>
                        <td>${row.customer_name}</td>
                        <td class="card_amount">${__number_f(row.cash_amount, false, false, currencyPrecision)}</td>
                        <td>${row.cash_note}</td>
                        <td>
                  ...
                  $("#cash_deposit_table_body").append(`
                      <tr> 
                       ...
                       row.account +
                        ...
                        __number_f(row.cash_deposit_amount, false, false, currencyPrecision) +
                        `</td>
                        <td>` +
                        row.time +
                        `</td>
                 ...
                 add_payment(row.cheque_amount);
                  $("#cheque_table_body").append(`
                      <tr> 
                        ...
                        `</td>
                        <td class="cheque_amount">` +
                        __number_f(row.cheque_amount, false, false, currencyPrecision) +
                        `</td>
                         <td>` +
                        row.cheque_note +
                        `</td>
                 ...
                 add_payment(row.expense_amount);
                  $("#expense_table_body").append(`
                      <tr> 
                        ...
                        __number_f(row.expense_amount, false, false, currencyPrecision) +
                        `</td>
                 ...
                 add_payment(row.shortage_amount);
                  $("#shortage_table_body").append(`
                      <tr> 
                        ...
                        __number_f(row.shortage_amount, false, false, currencyPrecision) +
                        `</td>
                        <td>` +
                        row.shortage_note +
                        `</td>
                 ...
                 add_payment(row.excess_amount);
                  $("#excess_table_body").append(`
                      <tr> 
                        <td></td>
                        <td class="shortage_amount">` +
                        __number_f(row.excess_amount, false, false, currencyPrecision) +
                        `</td>
                        <td>` +
                        row.excess_note +
                        `</td>
                 ...
                 add_payment(row.sub_total);
                  $("#credit_sale_table_body").append(`
                      <tr> 
                        ...
                        `</td>
                        <td>` +
                        __number_f(row.unit_price, false, false, currencyPrecision) +
                        `</td>
                        <td>` +
                        __number_f(row.qty, false, false, currencyPrecision) +
                        `</td>
                        <td class="credit_sale_amount">` +
                        __number_f(row.sub_total, false, false, currencyPrecision) +
                        `</td>
                        
                        <td class="credit_tbl_discount_amount">` +
                        __number_f(row.discount_total, false, false, currencyPrecision) +
                        `</td>
                        <td class="credit_tbl_total_amount">` +
                        __number_f(row.total, false, false, currencyPrecision) +
                        `</td>                        
                        <td>` +
                        row.note +
                        `</td>
            ...
            @if(!empty($temp_data->loan_payment_rows)) 
              ...
              loan_payment_rows.forEach(row => {
                 add_payment(row.loan_payments_amount);
                  ...
                  <tr>
                      <td>${row.bank_name}</td>
                        <td class="loan_payments_amount">${__number_f(row.loan_payments_amount, currencyPrecision)}</td>
                 ...
                 add_payment(row.cash_payment_amount);
                  ...
                  <tr>
                      <td>${row.cash_payment_loan_account_name}</td>
                        <td class="loan_payments_amount">${__number_f(row.cash_payment_amount, currencyPrecision)}</td>
            ...
            @if(!empty($temp_data->customer_loans_rows)) 
              ...
              customer_loans_rows.forEach(row => {
                console.log('customer', row)
                  ...
                  <tr>
                      <td>${row.customer_name}</td>
                        <td class="loan_payments_amount">${__number_f(row.customer_loan_amount, currencyPrecision)}</td>
    ...
    function handlePaymentSuccessDelete(row_id) {
        // Lancer un événement personnalisé global
        $(document).trigger("custom:payment_deleted", [row_id]);
    }
  </script>
