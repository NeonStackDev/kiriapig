...
<section class="content">
    ...
    <div class="">
        ...
        <div class="col-md-8 booking_add_room">
            ...
            <button type="button" class="btn btn-primary all-p-btn add-room"> @lang('messages.add')
                @lang('hms::lang.a_room')</button>
        </div>
        <div class="col-md-4">
            <table class="table table-bordered">
                <thead>
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <input type="checkbox" name="extras[{{ $extra->id }}][id]" class="extra"
                                value="{{ $extra->id }}" data-value="{{ $extra->price_per }}"
                                data-price="{{ $extra->price }}">
                            <input type="hidden" name="extras[{{ $extra->id }}][price]" value=""
                                id="{{ $extra->id }}">
                            {{ $extra->name }} / <span class="display_currency"
                                data-currency_symbol="true"> {{ $extra->price }}</span> -
            ...
            <div class="panel-body">
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <a class="tw-dw-btn tw-dw-btn-success tw-text-white apply_coupon">@lang('hms::lang.apply_coupon')</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="payment_row">
        ...
        <div class="hide payment_row" id="change_return_payment_data">
            <div class="col-md-6">
                <div class="form-group">
                    {!! Form::label('change_return_method', __('lang_v1.change_return_payment_method1') . ':*') !!}
                    <div class="input-group">
                        <span class="input-group-addon">
                            <i class="fas fa-money-bill-alt"></i>
                        </span>
            ...
            <div class="col-md-6">
                <div class="form-group">
                    {!! Form::label('change_return_account', __('lang_v1.change_return_payment_account1') . ':') !!}
                    <div class="input-group">
                        <span class="input-group-addon">
                            <i class="fas fa-money-bill-alt"></i>
                        </span>
    ...
    <div class="col-md-12 text-center">
        ...
        <button type="submit" class="btn btn-primary" name="submit_action" id="submit_action">
            @lang('cheque.save')
        </button>
    ...
    {!! Form::close() !!}
    {{-- model  --}}
    <div class="modal fade view_modal_room" tabindex="-1" role="dialog" aria-labelledby="gridSystemModalLabel">
    </div>
...
<script>
    $(document).ready(function() {
        ...
        $(".view_modal_room").on("show.bs.modal", function() {
            ...
            //     $.ajax({
            //         url: "{{ route('get_room_by_room_type') }}",
            //         method: 'GET',
            //         data: {
            //             'room_type_id': $(this).val(),
            //         },
            //         success: function(data) {
            //             let $roomSelect = $('#room_no');
            //             $roomSelect.empty(); // vide le select
            //             $roomSelect.append($('<option>', {
            //                 value: '',
            //                 text: 'Select an option'
            //             }));
            //             $.each(data, function(id, number) {
            //                 $roomSelect.append($('<option>', {
            //                     value: id,
            //                     text: number
            //                 }));
            ...
            $(document).ready(function() {
                ...
                $(document).on('change', '#type', function() {
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            console.log(result);
                            $('#dynamic-fields').html(result);
                            initializeDynamicFields(); // rebind if needed
                        },
                        error: function(xhr) {
                            console.error('Failed to load room type data:', xhr.responseText);
                            toastr.error('Error loading room type data. Please try again.');
                        }
                    });
                });


                // Function to initialize dynamic fields (if needed)
                function initializeDynamicFields() {
                    // Example: Initialize select2 or other plugins for dynamically added fields
                    $('#no_of_adult, #no_of_child, #room_no').select2({
                        placeholder: 'Select an option',
                        allowClear: true,
                    });
                }

                // Initial call to initialize dynamic fields on page load
        ...
        async function calculateAllPrice() {
            ...
            await Promise.all(
                $('.booking_add_room table tbody tr').map(async function() {
                    ...
                    const tr = $(this);
                    const price = parseFloat(tr.find('input[name^="rooms["][name$="][price]"]')
                        .val());
                    ...
                    tr.find('.price-td').text(__currency_trans_from_en(total_price.toFixed(2),
                        true));
                    tr.find('input[name^="rooms["][name$="][total_price]"]').val(total_price);
                    room_price += total_price;
                    person += parseFloat(tr.find(
                            'input[name^="rooms["][name$="][no_of_adult]"]').val()) +
                        parseFloat(tr.find('input[name^="rooms["][name$="][no_of_child]"]')
                            .val());
                })
            );
            ...
            $('.extra').each(async function() {
                ...
                if ($(this).prop('checked')) {
                    ...
                    if (per_type == 'per_day_per_person') {
                        extra_amount = parseFloat(extra_amount) + calculateDays() * person *
                            price;
            ...
            if ($('#coupon_code').val() != '') {
                if (total_discount > 0) {
                    ...
                    $('.total_discount_show').text(__currency_trans_from_en(total_discount.toFixed(2),
                        true));
                    $('.coupon-box').show();
                    $('.coupon-box').html("{{ __('hms::lang.applied_successfully') }}").css({
                        'font-weight': 'bold',
                        'font-size': '16px',
                        'color': 'green'
                    });
                    ...
                    $('.coupon-box').show();
                    $('.coupon-box').html("{{ __('hms::lang.something_went_wrong') }}").css({
                        'font-weight': 'bold',
                        'font-size': '16px',
                        'color': 'red'
                    });
        ...
        function calculateDays() {
            ...
            if (startDate.isValid() && endDate.isValid()) {
                ...
                var daysDifference = endDate.diff(startDate,
                    'days'); // Adding 1 to include both start and end days
                if (daysDifference <= 0) {
                    ++daysDifference;
                }
        ...
        function set_default_customer() {
            ...
            var exists = default_customer_id ? $('select#customer_id option[value=' + default_customer_id + ']')
                .length : 0;
            ...
            if ($("textarea#repair_defects").length > 0 && !customer_set) {
                let suggestions = [];
                if ($("input#pos_repair_defects_suggestion").length > 0 && $(
        ...
        $(document).on('change', '.payment_types_dropdown', function(e) {
            ...
            // Show/hide cheque boxes if element exists
            if ($('.pd_cheque_boxes').length) {
            ...
            // Get accounting module dropdown
            ...
            if (payment_type == 'advance') {
                ...
                // Disable accounting module for advance payments
                if (accounting_module_dropdown) {
                    accounting_module_dropdown.prop('disabled', true);
                    accounting_module_dropdown.closest('.form-group').addClass('hide');
                }
            } else {
                ...
                // Enable accounting module for non-advance payments
                if (accounting_module_dropdown) {
                    accounting_module_dropdown.prop('disabled', false);
                    accounting_module_dropdown.closest('.form-group').removeClass('hide');
                }
            }
        });

        // Keep the post-dated cheque handler separate
        $(document).on('change', '#update_post_dated_cheque', function() {
            ...
            $.ajax({
                ...
                url: '/accounting-module/get-account-group-name-dp',
                data: {
                    group_name: $(this).is(':checked') ? "direct_bank_deposit" : payment_type,
                    location_id: location_id
                },
                contentType: 'html',
                success: function(result) {
    ...
    $(document).on('click', 'button#submit_action', function(e) {
        if ($('.booking_add_room table tbody tr').length == 0 && $(
                'input[type="checkbox"][name^="extras["][name$="][id]"]:checked').length == 0) {
