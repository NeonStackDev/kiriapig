...
class AddPaymentController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $this->addDailyShortageExcess($settlement->id,$pump_operator_id,$business_id, $request->shift_ids);
        ...
        $settlement_card_payments = SettlementCardPayment::query()
            ...
            ->where(function ($query) use ($pump_operator_id) {
                $query->whereNull('daily_cards.id')
                    ->orWhere('daily_cards.pump_operator_id', $pump_operator_id);
            })
            ->select(
        ...
        $settlement_credit_sale_payments = SettlementCreditSalePayment::query()
            ...
            ->where(function ($query) use ($pump_operator_id) {
                $query->whereNull('daily_vouchers.id')
                    ->orWhere('daily_vouchers.operator_id', $pump_operator_id);
            })
            ->select(
