...

{
                    ...
                    if($row->description == 'updated'){
                        ...
                        foreach ($new as $key => $newValue) {
                            ...
                            if($key != 'created_at' && $key != 'updated_at' && $key != 'id'){
                                ...
                                if($key == 'payment_method' && $oldValue == 'Cash') $oldValue = 'Cash ';
    ...
    ->leftjoin('fuel_tanks', 'fuel_tanks.id', 'tank_sell_lines.tank_id')
    ->leftjoin('products', 'products.id', 'tank_sell_lines.product_id')
    ->leftjoin('settlements', 'settlements.id', 'transactions.invoice_no') // Add this join
...
\Log::info("settlement total{$query->get()}");
                        ...
                        function ($row) use ($first,$delete_settlement,$edit_settlement,$edit_settlement_no_change) {
                            ...
                            if ($row->status == 1) {
                                if (Str::startsWith($row->settlement_no, 'SET-SW')) {
                                    $html .= '<a class="btn  btn-danger btn-sm" href="' . action("\Modules\SettlementSW\Http\Controllers\SettlementSWController@index") . '">' . __("petro::lang.finish_settlement") . '</a>';
                                }else{
...

    {
        ...
        $other_sale_final_total = 0.00; $pump_other_sale_final_total = 0.00; $combinedOtherSales = [];
           ...
           if($active_settlement) {
            ...
            foreach ($active_settlement->other_sales as $ot_item) {
                ...
                $userOtherDetails[] = [
                    ...
                    'balance_stock'   => number_format($ot_item->balance_stock, 4, '.', ','),
                    'price'           => number_format($ot_item->price, $currency_precision),
                    'qty'             => number_format($ot_item->qty, 4, '.', ','),
                    'discount_type'   => $ot_item->discount_type,
                    'discount'        => number_format($ot_item->discount, $currency_precision),
                    'sub_total'       => number_format($ot_item->sub_total, $currency_precision),
    ...
    $business_id = request()->session()->get('user.business_id');
            ...
            if($request->shift_ids){
                // \Log::debug("save payment shift_ids: ",["shift_ids" => $request->shift_ids]);
                ...
                $pump_operator_other_sales = $pump_operator_total_other_sale->select('pump_operator_other_sales.*')->get();
            ...
            $settlement_total = $settlement->meter_sales->sum('sub_total') + $settlement->other_sales->sum('sub_total') + $settlement->other_incomes->sum('sub_total') + $settlement->customer_payments->sum('sub_total') + $pump_operator_total_other_sale;
            ...
            $total_sales_amount = $settlement->meter_sales->sum('sub_total') + $settlement->other_sales->sum('sub_total') + $pump_operator_total_other_sale;
            ...
            foreach ($pump_operator_other_sales as $pump_operator_other_sales_item) {
                ...
                if($pump_operator_other_sales_item->transaction_id == null || $pump_operator_other_sales_item->transaction_id !=$transaction->id){
                    $sell_line = $this->createSellTransactions($transaction, $pump_operator_other_sales_item, $business_id, $default_location,null,"pump_operator_other_sale");
                    PumpOperatorOtherSale::where('id', $pump_operator_other_sales_item->id)->update(['transaction_id' => $transaction->id]);
            ...
            $msg_template_wahtsapp = PetroWhatsAppTemplate::where('business_id',$business_id)->where('auto_send_sms',1)->first();
            ...
            //     $msg = $sms_data;//$msg_template_wahtsapp->sms_body;

            //    // $account = Account::find(1);
            //    // $bank_name = !empty($account) ? $account->name : "";

            //     // $msg = str_replace('{account}','cash', $msg);
            //     // $msg = str_replace('{amount}', 5000, $msg);
            //     // $msg = str_replace('{time}', "2:00 AM", $msg);
            //     // $msg = str_replace('{bank}', "awash", $msg);
            ...
            //         $clean_phone = $whatsapp_phone_no ;//preg_replace('/[^0-9]/', '', $phone);
            ...
            //         // Optional: Log or return the link
            //         \Log::info("WhatsApp URL: $whatsapp_url");

            //         // Or redirect user to WhatsApp URL (if this is a web route/controller)
            //          return redirect()->away($whatsapp_url);
            // }
            //$shift_ids = $request->shift_ids;
            return view('petro::settlement.print')->with(compact('settlement', 'business', 'pump_operator', 'customer_payments_tab', 'total_daily_collection', 'shift_ids'));
...

    {
        ...
        $settlement = Settlement::where('settlements.id', $id)->where('settlements.business_id', $business_id)
            ...
            ->select('settlements.*', 'pump_operators.name as pump_operator_name', 'pump_operator_assignments.shift_id')
...

    {
       ...
       $active_settlement = Settlement::where('id', $id)
        ...
        ->select('shift_number', 'shift_id')->get()->toarray();



        $other_sale_final_total = 0;  $pump_other_sale_final_total = 0;
...

    {
        ...
        try {
            ...
            $success = true;
            $optionHtml = ''; $firstOption = true;
            foreach ($assigned_pumps as $item) {
                $shift = PetroShift::findOrFail($item->shift_id);
...

    {
        ...
        $last_meter_sale = MeterSale::where(['pump_id' => $pump_id, 'shift_id'=> $shift_id])->orderBy('id', 'desc')->first();
        ...
        $ass = PumpOperatorAssignment::where('pump_id', $pump_id)->where('closed_in_settlement',0)
            ->where('shift_id', $shift_id)->orderBy('id', 'desc')->first();
...
public function getBalanceStockById(Request $request, $id)
{
    ...
    public function checkPreviousPumpSettlement()
    {
        try {
            ...
            // Get the current pump operator assignment
            $currentAssignment = PumpOperatorAssignment::where('shift_id',$shift_id)->first();
            ...
            // Find any previous unsettled assignment for the same pump
            $previousUnsettled = PumpOperatorAssignment::where('pump_id', $currentAssignment->pump_id)
                ->where('id', '<', $currentAssignment->id) // Look for older shifts
                ->where(function($query) {
                    $query->whereNull('settlement_id')
                        ->orWhere('closed_in_settlement', false);
                })
                ->orderBy('id', 'desc')
                ->first();

            if ($previousUnsettled) {
                $pump = Pump::select('pump_name')->find($previousUnsettled->pump_id);
                $operator = PumpOperator::select('name')->find($previousUnsettled->pump_operator_id);
                $pump_name = $pump ? $pump->pump_name : 'Pump';
                $operator_name = $operator ? $operator->name : 'Operator';
                return response()->json([
                    'status' => false,
                    'msg' => 'This pump ' . $pump_name . ' has a previous unsettled shift ' . $previousUnsettled->shift_number . ' with operator ' . $operator_name . '. Please settle it first.',
                ], 200);
            }
