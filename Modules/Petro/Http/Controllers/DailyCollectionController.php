...
//dd($dailyShiftOperators);

        $operatorShiftsMap = [];

        foreach ($dailyShiftOperators as $id => $name) {
            $shifts = PetroDailyShift::where('business_id', $business_id)
                ->where('status', 0)
                ->whereRaw("FIND_IN_SET(?, pump_operator_assigned)", [$id])
                ->pluck('shift_no')
                ->toArray();

            $operatorShiftsMap[$id] = $shifts;
        }
 ...
 //dd($assigned_operators);
        ...
        return view('petro::daily_collection.create')->with(compact('dailyShiftOperators','operatorShiftsMap','all','all_pump_operators','assigned','shiftNumbers','assigned_operators','locations', 'pump_operators', 'collection_form_no','default_location','shift_numbers'));
...
// assign pump operator to shift
    ...
    if ($dailyShift && $dailyShift->pump_operator_assigned) {
        $assigned_ids = explode(',', $dailyShift->pump_operator_assigned);
        $assigned_operators = PumpOperator::whereIn('id', $assigned_ids)
            ->pluck('name', 'id')
            ->toArray();
    } else {
        $assigned_operators = []; // fallback to empty if no shift found
    }
}
...
//dd($dailyShift->shift_no);
        return view('petro::daily_collection.edit')->with(compact('assigned','shiftNumbers','assigned_operators','locations', 'pump_operators','data','dailyShift'));
    }
    ...
    public function showDailyCashStatus()
    {
        ...
        return view('petro::daily_collection.partials.daily_cash_status', [
