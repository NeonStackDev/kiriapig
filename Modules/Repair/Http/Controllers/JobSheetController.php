...
{   
    ...
    public function __construct(RepairUtil $repairUtil, Util $commonUtil, CashRegisterUtil $cashRegisterUtil, ModuleUtil $moduleUtil,
        ContactUtil $contactUtil, ProductUtil $productUtil, BusinessUtil $businessUtil)
    {
       ...
       if (request()->ajax()) {
            ...
            return DataTables::of($job_sheets)
                ...
                ->editColumn('repair_no', function($row) {
                    ...
                    $add_invoice = '';
                    if (auth()->user()->can("repair.create")) {
                        $add_invoice = '<br><a href="' . action('SellPosController@create'). '?sub_type=repair&job_sheet_id='.$row->id. '&customer_id=' . $row->customer_id. '" class="cursor-pointer" data-toggle="tooltip" title="'.__('repair::lang.add_invoice').'">
    ...
    {   
        ...
        $brands = Brands::forDropdownRepair($business_id, false, true);
        $devices = Category::forDeviceDropdown($business_id);
        $repair_settings = $this->repairUtil->getRepairSettings($business_id);
        ...
        return view('repair::job_sheet.create')
            ->with(compact('status','repair_statuses', 'device_models', 'brands', 'devices', 'default_status', 'technecians', 'business_locations', 'types', 'customer_groups', 'walk_in_customer', 'repair_settings', 'contact_id'));
    }
    ...
    {   
        ...
        $status="1";
        return view('repair::job_sheet.create')
            ->with(compact('status','repair_statuses', 'device_models', 'brands', 'devices', 'default_status', 'technecians', 'business_locations', 'types', 'customer_groups', 'walk_in_customer', 'repair_settings'));
    }
    /**
    ...
    public function store(Request $request)
    {
        ...
        try {
            ////////////////////////////////////6029//////////////////////////////////
            $input = $request->only('reportStatus','contact_id', 'service_type', 'brand_id', 'device_id', 'device_model_id', 'security_pwd', 'security_pattern', 'serial_no', 'status_id', 'delivery_date', 'estimated_cost', 'product_configuration', 'defects', 'product_condition', 'service_staff', 'location_id', 'pick_up_on_site_addr', 'comment_by_ss', 'custom_field_1', 'custom_field_2', 'custom_field_3', 'custom_field_4', 'custom_field_5','warranty_number');
           //////////////////////////////////// end 6029//////////////////////////////////
    ...
    public function edit($id)
    {
        ...
        if (!(auth()->user()->can('user') || ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'repair_module') && auth()->user()->can('job_sheet.edit')))) {
            abort(403, 'Unauthorized action.');
        }
    ...
    public function destroy($id)
    {
        ...
        if (!(auth()->user()->can('user') || ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'repair_module') && auth()->user()->can('job_sheet.delete')))) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if (request()->ajax()) {
            try {
                ...
                $has_reviewed = $this->commonUtil->hasReviewed($job_sheet->delivery_date);
...
\Log::info('Product fetched:', (array) $product);
        $variation_name = '';
        $quantity = 1;
        $unit = '';
        $unit_price = 0;
    ...
    {   
        ...
        $business = Business::find($business_id);
        $repair_settings = json_decode($business->repair_settings, true);
        $vehicle=new_vehicle::where('id',$job_sheet->vehicle_id)->first();
        if($job_sheet->reportStatus=="1")
        {
            $barcode_string = ('Vehicle No:'. $vehicle->vehicle_no ?? '') . PHP_EOL . ('Brand:'. $job_sheet->brand->name ?? '') . PHP_EOL . 'Model:' . ($job_sheet->deviceModel->name ?? '') . PHP_EOL . ('Device:' . $job_sheet->device->name ?? '') . PHP_EOL . ('Status:' . $job_sheet->status->name) . PHP_EOL . ('Serial No:' . $job_sheet->serial_no ?? '') . PHP_EOL . ('Delivery Date:' . $job_sheet->delivery_date ?? '') . PHP_EOL . ('Product Condition:' .$job_sheet->product_condition ?? '') . PHP_EOL . ('Product Configration:' . $job_sheet->product_configuration ?? '') . PHP_EOL . ('Problem Reported By Client:' . $job_sheet->defects ?? '') . PHP_EOL . ('Estimated Cost:' . $job_sheet->estimated_cost ?? '');
        }
        else
        {
           if (isset($job_sheet->brand->name)) {
            
            $barcode_string = ('Brand:'. $job_sheet->brand->name ?? '') . PHP_EOL . ('Model:' . $job_sheet->deviceModel->name ?? '') . PHP_EOL . ('Device:' . $job_sheet->device->name ?? '') . PHP_EOL . ('Status:' . $job_sheet->status->name ?? '') . PHP_EOL . ('Serial No:' . $job_sheet->serial_no ) . PHP_EOL . ('Delivery Date:' . $job_sheet->delivery_date ?? '') . PHP_EOL . ('Product Condition:' .$job_sheet->product_condition ?? '') . PHP_EOL . ('Product Configration:' . $job_sheet->product_configuration ?? '') . PHP_EOL . ('Problem Reported By Client:' . $job_sheet->defects ?? '') . PHP_EOL . ('Estimated Cost:' . $job_sheet->estimated_cost ?? '');
           }
        }
        
      
        $qr = new DNS2D();
        if(isset($barcode_string)){
            $qr = $qr->getBarcodePNG($barcode_string, 'QRCODE');
             $barcode =  '<img style="width: 15%; height:15%" src="data:image/png;base64,' . $qr . '" alt="barcode"   />';
        ...
        $html = view('repair::job_sheet.print_pdf')
            ->with(compact('job_sheet', 'repair_settings', 'parts','barcode','vehicle'))->render();
        ...
        return view('repair::job_sheet.print_pdf')
            ->with(compact('job_sheet', 'repair_settings', 'parts','barcode'));
    }
