...
class RepairController extends Controller
{
    ...
    public function index()
    {
        ...
        if (!(auth()->user()->can('user') || ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'repair_module') && (auth()->user()->can('repair.view') || auth()->user()->can('repair.view_own')) ))) {
  ...
  //$sell=$sells->get();
                ...
                ->addColumn('total_remaining', function ($row) {
                 ...
                 ->editColumn('job_sheet_no', function($row) {
                    ...
                    if (auth()->user()->can("job_sheet.view_assigned") || auth()->user()->can("job_sheet.view_all") || auth()->user()->can("job_sheet.create")) {
                            $html = '<a href="' . action('\Modules\Repair\Http\Controllers\JobSheetController@show', ['job_sheet' => $row->job_sheet_id]) . '" class="cursor-pointer" target="_blank">
                                    '.$row->job_sheet_no.'
                                </a>';
                    }
    ...
    public function create()
    {
        ...
        $brands = Brands::forDropdownRepair($business_id);
    ...
    public function show($id)
    {
        ...
        if (!(auth()->user()->can('user') || ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'repair_module') && auth()->user()->can('repair.view')))) {
            abort(403, 'Unauthorized action.');
        }
    ...
    public function edit($id)
    {
        ...
        if (!(auth()->user()->can('user') || ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'repair_module') && auth()->user()->can('repair.update')))) {
            abort(403, 'Unauthorized action.');
        }
    ...
    public function editRepairStatus($repair_id)
    {
        ...
        if (!(auth()->user()->can('user') || ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'repair_module') && auth()->user()->can('repair_status.update')))) {
            abort(403, 'Unauthorized action.');
        }
