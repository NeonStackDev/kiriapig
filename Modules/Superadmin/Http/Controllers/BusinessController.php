...
class BusinessController extends BaseController
{
    ...
    public function manage($id, Request $request)
    {
        ...
        $expense_categories = ExpenseCategory::where('business_id', $id)
            ->pluck('name', 'id');

        $expense_account_type_id = AccountType::where('business_id', $id)->where('name', 'Expenses')->first();
        $expense_accounts = [];
        if ($this->moduleUtil->hasThePermissionInSubscription($id, 'access_account')) {
            if (!empty($expense_account_type_id)) {
                $expense_accounts = Account::where('business_id', $id)->where('account_type_id', $expense_account_type_id->id)->pluck('name', 'id');
            }
        }
        ...
        $last_expense_payment = SettlementExpensePayment::where('business_id', $id)
        ...
        // dd($manage_module_enable);
        $customers = Contact::customersDropdown($id, false);
        ...
        $last_customer_payment = CustomerPayment::where('business_id', $id)
        ...
        $services = Product::where('business_id', $id)->forModule('petro_settlements')->where('enable_stock', 0)->pluck('name', 'id');

        $customer_payment_settlement_no = $prefix . (1 + $count);
       
        return view('superadmin::business.manage')->with(compact('services','customer_payment_settlement_no','customers','active_settlement','expense_payment_settlement_no','expense_accounts','expense_categories','fonts','account_groups','subscription','customer_credit_notification_type',
        ...
        try {
           ...
           $addto_data['hms_addto'] = (!empty($module_activation_data['hms_module_length']) && !empty($module_activation_data['hms_module_interval'])) ? ($this->returnDays($module_activation_data['hms_module_interval']) * $module_activation_data['hms_module_length']) : $addto;
            ...
            foreach ($request->default_payment_accounts as $key => $default_payment_account) {
                // Fetch the existing default_payment_accounts from the database
                $businessLocation = BusinessLocation::find($key);
                $existingPayments = json_decode($businessLocation->default_payment_accounts, true) ?? []; // Convert JSON to array
            
                $payments = []; // New payments array
            
                foreach ($default_payment_account['name'] as $k => $one) {
                    $payments[$one] = [
                        "is_enabled" => $default_payment_account['is_enabled'][$k] ?? 0,
                        "is_purchase_enabled" => $default_payment_account['is_purchase_enabled'][$k] ?? 0,
                        "is_sale_enabled" => $default_payment_account['is_sale_enabled'][$k] ?? 0,
                        "is_expense_enabled" => $default_payment_account['is_expense_enabled'][$k] ?? 0,
                        "is_purchase_return_enabled" => $default_payment_account['is_purchase_return_enabled'][$k] ?? 0,
                        "is_sale_return_enabled" => $default_payment_account['is_sale_return_enabled'][$k] ?? 0,
                        "is_custom" => $default_payment_account['is_custom'][$k] ?? 0,
                        "account" => $default_payment_account['account'][$k] ?? 81,
                    ];
                }
            
                // Merge existing JSON data with the new data
                $mergedPayments = array_merge($existingPayments, $payments);
            
                // Debugging: Check the final JSON structure before updating
                // dd($existingPayments, $payments, $mergedPayments);
            
                // Update the database with the merged JSON
                BusinessLocation::where('id', $key)->update([
                    'default_payment_accounts' => json_encode($mergedPayments)
                ]);
            }
...
//            }
            ...
            $other_permissions_array = array(
                ...
                'default_setting','same_order_no','acc_no_manually','fixed_assets','stock_taking','fixed_conversion','list_realized_cheques',
            ...
            if (!empty($subscription)) {
                ...
                if(!isset($package_details["settlement_sw_module"])) {
                ...
                if ($request->has('settlement_sw_module')) {
                    $package_details["settlement_sw_module"] = strtotime($module_activation_data['settlement_sw_module_expiry_date']) > time() 
                        ? 1 
                        : ($package_details["settlement_sw_module"] ?? 0);
                } else {
                ...
                if ($request->has('stock_transfer') && $request->input('stock_transfer') == 1) {
                    ...
                    $package_details["purchstock_transferase"] = 0;
                }
