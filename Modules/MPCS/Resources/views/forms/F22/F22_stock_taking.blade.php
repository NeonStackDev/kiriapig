      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-secondary" data-dismiss="modal">@lang('mpcs::lang.cancel')</button>
              <button type="button" class="btn btn-primary" id="apply-date-range">@lang('mpcs::lang.apply')</button>
            </div>
        </div>
      </div>
    </div>
 ...
 <script type="text/javascript">
    ...
    // Update DataTable page length
            ...
            $('#form_16a_date').daterangepicker(dateRangeSettings, function(start, end, label) {
                ...
                $('#form_16a_date').val(start.format(moment_date_format));
            });
     ...
     form_22_table = $('#form_22_table').DataTable({
        ...
        columns: [
            ...
            // { data: 'DT_Row_Index', name: 'DT_Row_Index', width: "26px" },
            // { data: 'sku', name: 'sku' },
            // { data: 'product', name: 'product' },
            // { data: 'current_stock', name: 'current_stock', width: '55px' },
            // { data: 'stock_count', name: 'stock_count', width: '55px' },
            // { data: 'unit_purchase_price', name: 'unit_purchase_price', width: '80px' },
            // { data: 'total_purchase_price', name: 'total_purchase_price', width: '80px' },
            // { data: 'unit_sale_price', name: 'unit_sale_price', width: '80px' },
            // { data: 'total_sale_price', name: 'total_sale_price', width: '80px' },
            // { data: 'qty_difference', name: 'qty_difference', width: '40px' },
        ],
        fnDrawCallback: function(oSettings) {
        ...
        form_22_table.columns.adjust();
          ...
          $(document).on('keyup', '.stock_count', function() {
            ...
            form_22_last_verified_table = $('#form_22_last_verified_table').DataTable({
                ...
                initComplete: function (settings, json) {
                    var table_info = form_22_last_verified_table.page.info();
                    for (i = 0; i < table_info.pages; i++) {
                ...
                //   $('#lf_footer_total_purchase_price').text(__number_f(purchaseTotal));
                //   $('#lf_footer_total_sale_price').text(__number_f(saleTotal));
                // },
            ...
            function lastFormCalculateTotals() {
                let lf_pgrand = 0.00;
                let lf_sgrand = 0.00;
                let info = form_22_last_verified_table.page.info();
                ...
                if (info.page === 0) {
                    lf_pgrand = lf_ppage_totals[0];
                    lf_sgrand = lf_spage_totals[0];
                } else {
                ...
                if (info.page > 0) {
                    ...
                    $('#lf_pre_total_purchase_price').text(__number_f(0, false, false, __currency_precision));
                    $('#lf_pre_total_sale_price').text(__number_f(0, false, false, __currency_precision));
                }
    ...
    // Collect all table data as JSON
    let tableData = [];
    form_22_table.rows({ page: 'all' }).nodes().to$().each(function() {
        let row = $(this);
        let rowData = {};
    ...
    // Get form data
    let formData = $('#f22_form').serializeArray();
    ...
    $.ajax({
        ...
        $(document).on('click', '.ranges ul li[data-range-key="Custom Date Range"]', function() {
            // $('#f22DateRangeModal').modal('show');
        ...
        $('#custom_date_apply_button').on('click', function () {
            ...
            if (startDate.length === 10 && endDate.length === 10) {
                ...
                $('#form_16a_date').val(formattedStartDate + ' - ' + formattedEndDate);
                $('#f22DateRangeModal').modal('hide');
