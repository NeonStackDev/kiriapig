...
class MPCSController extends Controller
{
    ...
    public function From9A()
    {
        ...
        // Get form settings
        $form_settings = DB::connection('mysql')
            ->table('mpcs_9a_form_settings')
        ...
        if (!empty($form_settings) && !empty($form_settings->starting_number) && !empty($form_settings->date)) {
            ...
            $form_number = $form_settings->starting_number + $days_passed;
        }
    ...
    public function From9CCR()
    {
        ...
        $currency_precision = $business ? $business->currency_precision : 2;
            ...
            if ($request->ajax()) {
                ...
                if (!$formF22Exists && $setting) {
                    $previous_totals = DB::table('transactions')
                        ...
                        ->where('account_transactions.business_id', $business_id)
                        ->where('transactions.is_credit_sale', 0)
                        ->whereDate('transactions.transaction_date', '>=', $setting_start_date)
                        ->whereDate('transactions.transaction_date', '<=', $previous_day)
                        ->select(
                            'products.name as product',
                            'transactions.invoice_no as billno',
                ...
                if ($setting) {
                    ...
                    $uniqueTransactionDatesCount = DB::table('transactions')
                        ...
                        ->whereDate('transaction_date', '>=', $setting_start_date)
