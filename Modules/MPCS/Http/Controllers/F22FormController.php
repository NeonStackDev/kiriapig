...
class F22FormController extends Controller
{
    ...
    public function getF22FormList(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($header)
                ->addIndexColumn()
                ->orderColumn('created_at', 'form_f22_headers.created_at $1')
                ...
                ->addColumn('total_stock_lose_purchase', function ($row) use ($currency_precision) {
                    $total = FormF22Detail::where('form_no', $row->form_no)
                        ...
                        ->value('total');
                    return number_format($total, $currency_precision);
                })
    ...
    public function view($header_id)
    {
        ...
        return view('mpcs::forms.F22.partials.view_byID_f22_form')->with(compact('header', 'details', 'settings'));
    }
...
\Log::info("Stock taking{$purchases->get()}");
            return DataTables::of($purchases)
                ...
                ->addColumn('qty_difference', function ($row) use($qty_precision, $business_id) {
                  ...
                  $stockCount = number_format($row->alert_quantity);
                    ...
                    return '<input class="form-control qty_difference" name="f22[' . $row->product_id . '][qty_difference]" id="f22[' . $row->product_id . '][qty_difference]" style="width: 100%;" name="qty_difference" value="'.$difference.'" >';
                })
                ->editColumn('sku', function ($row) {
    ...
    public function saveF22Form(Request $request)
    {
        ...
        try {
            ...
            \Log::info('Received F22 form data:', ['form_data' => $formData]);
            \Log::info('Received table data:', ['table_data' => $tableData]);
            ...
            $headerData = [
                ...
                'location_id' => $formData['f22_location_id'] ?? null,
            ...
            // Fetch settings and link account
            $settings = MpcsFormSetting::where('business_id', $business_id)->first();
            $linkAccount = FormF22LossGain::where('business_id', $business_id)->first();
            ...
            foreach ($tableData as $rowKey => $rowData) {
                try {
                    ...
                    $difference = (isset($item['stock_count']) && isset($item['current_stock']))
                        ? (float)$item['stock_count'] - (float)$item['current_stock']
                        : 0;
                    ...
                    $purchase_price_total = $stock_count * $unit_purchase_price;
                    $sales_price_total = $stock_count * $unit_sale_price;
                    ...
                    //     'unit_purchase_price' => $item['unit_purchase_price'] ?? 0,
                    //     'unit_sale_price' => $item['unit_sale_price'] ?? 0,
                    //     'purchase_price_total' => $item['total_purchase_price'] ?? 0,
                    //     'sales_price_total' => $item['total_sale_price'] ?? 0,
                    //     'difference_qty' => $difference,
                    //     'status' => 1
                    // ];
                    ...
                    if (!empty($settings) && $settings->current_stock_aa_onstocktaking == 1 && isset($item['variation_id'])) {
                    ...
                    if (!$product) {
                        \Log::warning('Product not found', ['product_name' => $item['product']]);
                        continue;
                    }
    
                    $variation = Variation::where('product_id', $product->id)->first();
                    $unitPrice = $variation ? $variation->default_purchase_price : 0;
    
                    if ($difference > 0) {
                        $totalLoss += abs($difference) * $unitPrice;
                    } elseif ($difference < 0) {
                        $totalGain += abs($difference) * $unitPrice;
                    }
                } catch (\Exception $e) {
    ...
    protected function createStockTransaction($businessId, $locationId, $formNo, $subType, $amount, $linkAccount, $description)
    {
        ...
        $debitAccount = $subType === 'shortage' ? $linkAccount->stock_loss_account : $linkAccount->stock_gain_account;
        $creditAccount = $subType === 'shortage' ? $linkAccount->stock_gain_account : $linkAccount->stock_loss_account;
