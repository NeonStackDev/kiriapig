...
$last_purchase_line = PurchaseLine::latest('id')
        ->first();
        $variation_price = VariationPrice::latest('id')
        ->first();
 \Log::warning("purchase line many" . $last_purchase_line);
  \Log::warning("variation_price line many" . $variation_price);
      if ($last_purchase_line) {
        $last_purchase_line->variation_id = $variation_price->id;
        $last_purchase_line->save();
        \Log::info("Updated purchase line ID {$last_purchase_line->id} to new variation ID {$variation_price->id}");
    } else {
        \Log::warning("No purchase line found for variation ID: " . $original_variation->id);
    }
        }
