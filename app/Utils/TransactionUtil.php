    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
{
    try {
        ...
        \Log::info("Sell transaction created successfully.", ['transaction_id' => $transaction->id]);
     ...
     function floattostr( $val )
{
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $unit_price = floatval($line->unit_price_before_discount);
            ...
            $item_tax = 0;
            if($tax_details) {
                $item_tax = ($unit_price_after_discount * $tax_details->amount) / 100;
            }
            $unit_total_price = $unit_price_after_discount + $item_tax;
            ...
            $line_array = [
                ...
                'units' => $unit_name,
                'unit_price' => $this->num_f($unit_price, false, $business_details),
                'tax' => $this->num_f($item_tax, false, $business_details),
                'tax_unformatted' => $item_tax,
                ...
                'unit_price_inc_tax' => $this->num_f($unit_tax_price, false, $business_details),
                'unit_price_exc_tax' => $this->num_f($unit_price, false, $business_details),
                'price_exc_tax' => $line->quantity * $unit_price,
                'unit_price_before_discount' => $this->num_f($unit_price, false, $business_details),
                //Fields for 4th column
                'line_total' => $this->num_f($unit_total_price * $line->quantity, false, $business_details),
                'inital_unit_price' => $this->num_f($unit_price, false, $business_details), 
                'default_sale_price' => $this->num_f($variation->default_sell_price, false, $business_details),
                'sub_total_final' => $this->num_f($unit_total_price, false, $business_details)
            ];
            $temp = [];
            ...
            $line_array['line_discount'] = $unit_discount;
    ...
    public function calculatePaymentStatus($transaction_id, $final_amount = null,$amount =null , $paymentMethod = null)
    {
        ...
        // ðŸ‘‡ Check for credit purchase first
        if ($paymentMethod === 'credit_purchase') {
            $status = 'due'; // Always due in credit purchase
        } else {
...
// and then you can get query log
    ...
    public function createOrUpdateSellLinesSettlement($transaction, $product_id, $variation_id, $location_id, $meter_sale)
    {
        $sell_line_data = array(
            ...
            'line_discount_type' => $meter_sale->discount_type ?? 'fixed',
    ...
    public function getCategoryAccountId($category_id, $group)
    {
        $business_id = request()->session()->get('business.id');
        if ($group == 'cogs') {
            $categorie = Category::where('business_id', $business_id)->where('id', $category_id)->select('cogs_account_id')->first();
            
            return $categorie->cogs_account_id ?? null;
        }
        if ($group == 'sale_income') {
            $categorie = Category::where('business_id', $business_id)->where('id', $category_id)->select('sales_income_account_id')->first();
            return $categorie->sales_income_account_id ?? null;
        }
    }
