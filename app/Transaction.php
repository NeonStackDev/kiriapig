...
class Transaction extends Model
{
    ...
    public function account_transctions()
    {
        return $this->hasMany(\App\AccountTransaction::class)->where('sub_type', 'ledger_show')->first();
    }
    ...
    public function accountTransactionsRelation()
    {
        // return $this->hasMany(\App\AccountTransaction::class)->where('sub_type', 'ledger_show')->first();
        return $this->hasOne(\App\AccountTransaction::class, 'transaction_id');
    }
	
	//asif
	
	public function settlementCreditSalePayment()
    {
        return $this->hasOne(\App\SettlementCreditSalePayment::class, 'transaction_id');
    }
    ...
    public static function transactionTypes()
    {
        return  [
            ...
            'fleet_opening_balance' => __('fleet::lang.ob_of_to'),
            'payment' => __('lang_v1.payment'),
            'cheque_return' => __('lang_v1.cheque_return'),
            'route_operation' => __('fleet::lang.route_operations')
    ...
    {
        $business_id = request()->session()->get('user.business_id');
        $payment_ref_numbers = Transaction::leftjoin('transaction_payments', 'transactions.id', 'transaction_payments.transaction_id')->where('transactions.business_id', $business_id)->where('type','!=', 'purchase')->whereNotNull('transaction_payments.payment_ref_no')->pluck('transaction_payments.payment_ref_no', 'transaction_payments.payment_ref_no');
