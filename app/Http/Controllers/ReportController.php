...
<ul class="dropdown-menu dropdown-menu-right" role="menu">';
    ...
    public function getDailyReport(Request $request)
    {
        ...
        $sell_details = $sell_details->get();
        
        $sales = $sell_details->groupBy('sub_category_id')->map(function ($items, $sub_category_id) {
            ...
            foreach ($items as $line) {
                ...
                // Calcul du montant en fonction du type de remise
                $line_total = $line->unit_price_inc_tax * $qty;

                if ($line->line_discount_type === 'percentage') {
                    $discount = ($line_total * $line->line_discount_amount) / 100;
                } else {
