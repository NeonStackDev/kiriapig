...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if(empty($default_store) && $this->moduleUtil->isSubscribed($business_id) && auth()->user()->hasRole('Admin#' .$business_id)){
            $output = [
                'success' => 0,
        ...
        $currency = ($bussiness_currency)?$bussiness_currency->currency:Currency::where('id', $business_id)->first();
        if(is_null($currency)){
