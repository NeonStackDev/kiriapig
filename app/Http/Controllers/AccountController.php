...
try{
           ...
           $accounts = $accounts->select([
            ...
            return DataTables::of($modifiedAccounts)
                ...
                ->editColumn('operation_date', function ($row) {
                    ...
                    // } else {
                    return $this->commonUtil->format_date($row->created_at, false); // Modified By iftekhar
                    // }
                })
                    ...
                    if($row->txnType=='advance' && $row->transaction_id == null && $row->employee_advance_id != null):
                        ...
                        elseif ( !empty($row->transaction->type) &&  $row->transaction->type == 'hms_booking'){
                            $line = DB::table('hms_booking_lines')
                                    ->where('transaction_id', $row->transaction->id)
                                    ->first();
                            $room = HmsRoom::find($line->hms_room_id ?? null);
                            $details .= '<b>HMS Bill No:</b>' . $row->transaction->ref_no . '<br>';
                            $details .= '<b>Room Nos: </b>' . ($room->room_number ?? '') . '<br>';
                            $details .= '<b>' . __('contact.customer') . ':</b> ' . $row->transaction->contact->name . '<br>';
                        }
    ...
    public function getChequeList(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            // if($contact){
            //     $query = $query->where('contacts.id',$contact);
            // }
    ...
    public function postDeposit(Request $request)
    {
        ...
        try {
            ...
            $note = $request->input('note');
            $daily_shift_no = $request->input('daily_shift_no');
            $cheque_number = $request->input('cheque_number');
            $uploadFile = null;
            //upload file
            if (!file_exists('./public/img/account_transaction/' . $business_id)) {
