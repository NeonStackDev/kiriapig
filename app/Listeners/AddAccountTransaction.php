        ...
        foreach ($sell_lines as $sale) {
            ...
            if ($sale->quantity >= 0) { //not include pos page return 
                ...
                if (!empty($account_id)) {
                    $discount = 0;
                    $tax_amount = 0;
                    $unit_price_ex_tax = round(floatval($sale->unit_price_before_discount), 2);
                    
                    if($sale->line_discount_amount > 0){
                        if($sale->line_discount_type == 'fixed'){
                            $discount = round($sale->line_discount_amount / floatval($sale->quantity), 2);
                        }else{
                            $discount = ($sale->line_discount_amount / 100) * $unit_price_ex_tax;
                        }
                    }
                    $discounted_amount = $unit_price_ex_tax - $discount;

                    if($sale->tax_id){
                        $tax = DB::table('tax_rates')->where('id', $sale->tax_id)->first();
                        $tax_amount = ($tax->amount / 100) * $discounted_amount;
                    }

                    $amount = ($discounted_amount + $tax_amount) * floatval($sale->quantity);
                    
                    $account_transaction_data = [
                        'amount' => abs($amount), // Sales Income to use price including tax
                        'account_id' => $account_id,
                        'type' => $type,
                        'sub_type' => $sub_type,
                        'operation_date' => $transaction->transaction_date,
    ...
    public function manageStockAccount($transaction, $account_transaction_data, $trans_type, $amount, $sub_type = null)
    {
        ...
        if ($transaction->type == 'sell') {
            $sell_lines = TransactionSellLine::leftjoin('products', 'transaction_sell_lines.product_id', 'products.id')
                ...
                ->leftjoin('variations', 'product_variations.id', 'variations.product_variation_id')
                ->where('transaction_id', $transaction->id)
                ->select('transaction_sell_lines.*', 'products.category_id', 'products.enable_stock', 'products.stock_type', 'products.sub_category_id', 'variations.dpp_inc_tax')
                ->get();
            ...
            foreach ($sell_lines as $sale) {
                if ($sale->quantity >= 0) { //not include pos page return 
                    ...
                    if ($sale->enable_stock) {
                        ...
                        $account_transaction_data['amount'] = abs($sale->quantity * $sale->dpp_inc_tax); // changed to include tax
